...
| `SQLiteKit`    | `sqlite3` | `sqlite3` shell port over the **vendored** SQLite amalgamation (`stephencelis/CSQLite`, pinned `3.50.4` — engine consumed as a package, no blob in-repo). The `SQLiteKit` SDK is a thin `SQLiteDatabase` wrapper (open / `evaluate` / `execute` / `tableNames` / `schemaSQL` / introspection) plus a `ResultFormatter` rendering result sets in `list` / `csv` / `line` / `column` / `json` modes. The `sqlite3` CLI runs SQL from a trailing argument, stdin, or a line-buffered REPL (`-csv` / `-json` / `-line` / `-column` / `-header` / `-separator` / `-nullvalue` / `-readonly` / `-init` / `-cmd`), and dispatches dot-commands `.tables .schema .databases .indexes .mode .headers .separator .nullvalue .read .open .show .help .quit`. Database / `.read` / `.open` / `-init` paths go through ShellKit's `resolve` + `authorize` sandbox gate. Exit codes mirror real sqlite3 (0 ok, 1 error). See issue #43. |
