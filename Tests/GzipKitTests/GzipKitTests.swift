...
@Suite struct GzipKitTests {
    ...
    @Test func compressFileProducesGzSuffix() throws {
        let work = tempDir()
        defer { try? FileManager.default.removeItem(at: work) }
        let source = work.appendingPathComponent("payload.txt")
        try Data("payload\n".utf8).write(to: source)

        let dest = try Gzip.compressFile(at: source, keepInput: true)
    ...
    @Test func decompressFileRefusesUnknownSuffix() throws {
        let work = tempDir()
        defer { try? FileManager.default.removeItem(at: work) }
        let source = work.appendingPathComponent("file.bin")
        try Data([0x1f, 0x8b]).write(to: source)
        #expect(throws: GzipKitError.self) {
            _ = try Gzip.decompressFile(at: source)
        }
    }

    @Test func decompressFileMapsTgzToTar() throws {
        let work = tempDir()
        defer { try? FileManager.default.removeItem(at: work) }
        ...
        let dest = try Gzip.decompressFile(at: tgz)
