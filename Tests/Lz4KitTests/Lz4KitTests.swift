...
@Suite struct Lz4KitTests {
    ...
    @Test func roundTripIncompressible() throws {
    ...
    @Test func compressFileProducesLz4Suffix() throws {
        let work = tempDir()
        defer { try? FileManager.default.removeItem(at: work) }
        let source = work.appendingPathComponent("payload.txt")
        try Data("payload\n".utf8).write(to: source)

        let dest = try Lz4.compressFile(at: source, keepInput: true)
    ...
    @Test func decompressRejectsTruncated() throws {
        ...
        #expect(throws: Lz4KitError.self) {
            _ = try Lz4.decompress(truncated)
        }
    }

    @Test func decompressRejectsEmptyInput() throws {
        #expect(throws: Lz4KitError.self) {
            _ = try Lz4.decompress(Data())
        }
    }

    @Test func decompressRejectsBadMagic() throws {
        #expect(throws: Lz4KitError.self) {
            _ = try Lz4.decompress(Data([0xDE, 0xAD, 0xBE, 0xEF, 0x01, 0x02, 0x03]))
        }
    }
}
#endif
