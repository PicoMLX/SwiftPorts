...
@Suite struct TarKitTests {
    ...
    @Test func roundTripPlainTar() throws {
        let workDir = tempDir()
    ...
    @Test func roundTripGzippedTar() throws {
        let workDir = tempDir()
        defer { try? FileManager.default.removeItem(at: workDir) }
    ...
    @Test func extractRespectsStripComponents() throws {
        let workDir = tempDir()
        defer { try? FileManager.default.removeItem(at: workDir) }
        let archiveURL = workDir.appendingPathComponent("out.tar")
    ...
    @Test func listFromDataParsesGzipAutomatically() throws {
        let workDir = tempDir()
        defer { try? FileManager.default.removeItem(at: workDir) }
        let archiveURL = workDir.appendingPathComponent("out.tar.gz")
        ...
        let entries = try Archive.list(data: bytes)
        #expect(entries.contains { $0.path == "a.txt" })
    }

    @Test func extractSkipsExistingWhenOverwriteFalse() throws {
        let workDir = tempDir()
        defer { try? FileManager.default.removeItem(at: workDir) }
        let archiveURL = workDir.appendingPathComponent("out.tar")
